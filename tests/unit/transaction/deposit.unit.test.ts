...
describe('validateDepositAmount — EP + BVA ($0.01 .. $1,000,000.00)', () => {
  ...
  describe('Invalid partition: MIN DECIMAL .. $0.00 (negative and zero amounts)', () => {
    it.each<[string, Decimal]>([
      ...
      ['BV -$0.01 (just below lower boundary)', new Decimal('-0.01')],
      ['BV $0.00 (lower boundary — still invalid)', new Decimal('0')],
  ...
  describe('Valid partition: $0.01 .. $10,000.00 (accepted, NOT flagged for AML)', () => {
    it.each<[string, Decimal]>([
      ...
      ['BV $9,999.99 (just below AML threshold)', new Decimal('9999.99')],
      ['BV $10,000.00 (AML threshold — last NOT flagged)', new Decimal('10000.00')],
  ...
  describe('Valid partition: $10,000.01 .. $1,000,000.00 (accepted, flagged for AML)', () => {
    it.each<[string, Decimal]>([
      ['BV $10,000.01 (first AML-flagged amount)', new Decimal('10000.01')],
      ['BV $10,000.02 (just above AML threshold)', new Decimal('10000.02')],
  ...
  describe('Invalid partition: $1,000,000.01 .. MAX DECIMAL (above maximum)', () => {
    it.each<[string, Decimal]>([
      ['BV $1,000,000.01 (just above upper boundary)', new Decimal('1000000.01')],
